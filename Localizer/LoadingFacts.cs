...
{
	public static class LoadingFacts
	{
		...
		public static string GetRandomFactNoDate(Random random, bool isRhubarb = false) {
			var ran = random.Next(9);
			return ran <= 1 ? GetRandomFunnyFact(random) : ran <= 3 ? GetRandomFact(random) : isRhubarb ? GetRandomRhubarbFact(random) : GetRandomFact(random);
		}

		public static string GetRandomFact(Localisation loc, bool isRhubarb = false) {
			...
			var random = GetRandomFactNoDate(randomGen, isRhubarb);
